...
def test_mean_collector(ref_graph, mock_repository):
    ...
    means, _ = collector.collect_values(ref_graph, False, [3, 2, 1, 0])
...
def test_shell_size_order(ref_graph, mock_order_repository):
    ...
    means, _ = collector.collect_values(ref_graph, False, [0, 1, 2, 3])
    assert means[1] == ([0.34], [1.0])
...
def test_caching_collector0(ref_graph, mock_repository):
    ...
    means0, _ = collector.collect_values(ref_graph, False, [3, 2, 1, 0])

    for shell, chdct in mock_repository.charges_iacm.items():
        for key in chdct.keys():
            chdct[key].append(1)

    means1, _ = collector.collect_values(ref_graph, False, [3, 2, 1, 0])
